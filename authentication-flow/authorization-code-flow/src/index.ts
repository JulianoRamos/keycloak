...
app.get("/login", (req, res) => {
  const nonce = crypto.randomBytes(16).toString("base64"); // Protege contra Replay Attack
  const state = crypto.randomBytes(16).toString("base64"); // Protege contra CSRF Attack
  ...
  const url = `http://localhost:8080/realms/tog-realm/protocol/openid-connect/auth?${loginParams.toString()}`;
  console.log(url);
  res.redirect(url);
});
...
app.get("/logout", (req, res) => {
  ...
  const url = `http://localhost:8080/realms/tog-realm/protocol/openid-connect/logout?${logoutParams.toString()}`;
  res.redirect(url);
});
...
app.get("/callback", async (req, res) => {
  ...
  const url = `http://host.docker.internal:8080/realms/tog-realm/protocol/openid-connect/token`;
